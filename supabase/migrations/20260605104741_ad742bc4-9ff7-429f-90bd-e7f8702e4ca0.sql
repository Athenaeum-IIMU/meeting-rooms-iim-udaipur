...
BEGIN
  ...
  WHERE booking_id IN (SELECT id FROM public.bookings WHERE date < CURRENT_DATE - INTERVAL '2 days');
  ...
  -- NEW: also purge past admin-blocked slots so they don't haunt the calendar
  DELETE FROM public.blocked_slots WHERE date < CURRENT_DATE - INTERVAL '2 days';
END;
$function$;