...
DECLARE
  ...
  v_bookings_html TEXT := '';
  v_blocked_html TEXT := '';
  v_bookings_text TEXT := '';
  v_blocked_text TEXT := '';
  v_count_b INT := 0;
  v_count_s INT := 0;
  v_row RECORD;
  ...
  v_html TEXT;
BEGIN
  -- Gather bookings about to be deleted
  FOR v_row IN
    SELECT b.id, b.title, b.date, b.start_time, b.end_time, b.status,
           ...
           COALESCE(
             (SELECT string_agg(bm.email, ', ' ORDER BY bm.email)
              FROM public.booking_members bm WHERE bm.booking_id = b.id),
             ''
           ) AS members
    FROM public.bookings b
    LEFT JOIN public.rooms r ON r.id = b.room_id
    LEFT JOIN public.profiles p ON p.user_id = b.user_id
    WHERE b.date < v_cutoff
    ORDER BY b.date, b.start_time
  LOOP
    v_count_b := v_count_b + 1;
    v_bookings_html := v_bookings_html ||
      '<tr>' ||
      '<td style="padding:6px 8px;border:1px solid #e5e7eb;">' || v_row.date || '</td>' ||
      '<td style="padding:6px 8px;border:1px solid #e5e7eb;">' ||
        to_char(v_row.start_time,'HH24:MI') || '–' || to_char(v_row.end_time,'HH24:MI') || '</td>' ||
      ...
      '<td style="padding:6px 8px;border:1px solid #e5e7eb;">' || v_row.members || '</td>' ||
      '</tr>';
    v_bookings_text := v_bookings_text || v_row.date || ' ' ||
      to_char(v_row.start_time,'HH24:MI') || '-' || to_char(v_row.end_time,'HH24:MI') || ' | ' ||
      v_row.room_name || ' | ' || v_row.title || ' | ' || v_row.organizer ||
      ' | status=' || v_row.status || ' | members=' || v_row.members || E'\n';
  END LOOP;

  -- Gather blocked slots about to be deleted
  FOR v_row IN
  ...
  LOOP
    ...
    v_blocked_html := v_blocked_html ||
      ...
      '<td style="padding:6px 8px;border:1px solid #e5e7eb;">' || v_row.creator || '</td>' ||
      '</tr>';
    v_blocked_text := v_blocked_text || v_row.date || ' ' ||
      to_char(v_row.start_time,'HH24:MI') || '-' || to_char(v_row.end_time,'HH24:MI') || ' | ' ||
      v_row.room_name || ' | reason=' || COALESCE(v_row.reason,'') || ' | by ' || v_row.creator || E'\n';
  END LOOP;

  -- Send the daily archive email BEFORE deletion (only if there is something)
  IF v_count_b > 0 OR v_count_s > 0 THEN
    ...
    v_html := '<div style="font-family:Arial,sans-serif;max-width:900px;margin:0 auto;padding:24px;">'
           ...
           || '<p style="margin:6px 0 0;font-size:13px;opacity:.9;">Records dated before ' || v_cutoff || ' (about to be deleted)</p>'
           || '</div>'
    ...
    ELSE
      v_html := v_html || '<p style="color:#6b7280;">No bookings.</p>';
    END IF;
    ...
    ELSE
      v_html := v_html || '<p style="color:#6b7280;">No blocked slots.</p>';
    END IF;
    ...
    BEGIN
      ...
      VALUES (NULL, 'readers.library@iimu.ac.in',
              'Daily Cleanup Archive (' || v_cutoff || ')', 'sent');
    EXCEPTION WHEN OTHERS THEN
      INSERT INTO public.email_delivery_log (user_id, recipient, subject, status, error)
      VALUES (NULL, 'readers.library@iimu.ac.in',
              'Daily Cleanup Archive (' || v_cutoff || ')', 'failed', SQLERRM);
    END;
  END IF;
