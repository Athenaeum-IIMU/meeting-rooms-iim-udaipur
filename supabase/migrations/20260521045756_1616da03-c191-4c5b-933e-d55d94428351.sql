...
-- 1. Create the vault secret used by send-email function authentication
DO $$
DECLARE
  v_existing text;
BEGIN
  SELECT decrypted_secret INTO v_existing FROM vault.decrypted_secrets WHERE name='send_email_secret' LIMIT 1;
  IF v_existing IS NULL THEN
    PERFORM vault.create_secret(encode(gen_random_bytes(32), 'hex'), 'send_email_secret', 'Shared secret for send-email edge function');
  END IF;
END $$;

-- 2. Fix the hardcoded URL to point at this project's backend
